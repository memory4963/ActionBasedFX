...
skeleton, labels = ReadData.read_data("D:\\DataSet", dataset_size)
...
with tf.name_scope("full_weights1"):
    full_weights1 = tf.Variable(tf.truncated_normal([hidden_size, int(hidden_size / 4)], stddev=1e-2), dtype=tf.float32)
    Utils.variable_summaries(full_weights1)
with tf.name_scope("full_bias1"):
    full_bias1 = tf.Variable(tf.constant(1e-2, shape=[int(hidden_size / 4)]))
...
with tf.name_scope("full_weights2"):
    full_weights2 = tf.Variable(tf.truncated_normal([int(hidden_size / 4), int(hidden_size / 16)], stddev=1e-2), dtype=tf.float32)
    Utils.variable_summaries(full_weights2)
with tf.name_scope("full_bias2"):
    full_bias2 = tf.Variable(tf.constant(1e-2, shape=[int(hidden_size / 16)]))
    Utils.variable_summaries(full_bias2)
hidden = tf.nn.relu(tf.add(tf.matmul(hidden, full_weights2), full_bias2))
# softmax
with tf.name_scope("weights"):
    weights = tf.Variable(tf.truncated_normal([int(hidden_size / 16), class_num], stddev=1e-2), dtype=tf.float32)
